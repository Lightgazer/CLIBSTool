public sealed partial class MainWindow
{
    protected List<string> DumpText(FileInfo file, char[] code_page)
    {
        ...
        while (reader.BaseStream.Position < reader.BaseStream.Length)
        {
            ...
            else if ((int)num1 == 2024 || (int)num1 == 33280 || (int)num1 == 39936 || (int)num1 == 47104)
            { //some strange chars
                str += "■";
                reader.BaseStream.Position -= 2;
                strsq += "[" + reader.ReadByte() + "." + reader.ReadByte() + ".]";
            }
            else if ((int)num1 == 41985)
            { //actor tag
    ...
    protected void ScriptMaker(string script_dir, string out_txt)
    {
        ...
        char[] code_page;
        if (num.Length == 3)
            code_page = CodePage.GetCodePage(Int32.Parse(num[1]), num[2]);
        else
            code_page = CodePage.GetCodePage(Int32.Parse(num[1]));
        int index = 0;
