...
{
    private static ushort FindChar(char ch, char[] code_page, char[] second_code_page)
    {
    ...
    private static ushort FindChar(char ch, char[] code_page)
    {
        if (ch == 32) //space
    ...
    private static byte[] GetSqr(string parent, string name, int index)
    {
        StreamReader sqreader = new StreamReader(new FileStream(Path.Combine(parent, "Squares.txt"), FileMode.Open));
        string line;
    ...
    private static bool CheckSq(string script_file)
    {
        bool ret = true;
        string parent;
        if (script_file.Split(Path.DirectorySeparatorChar).Length > 1)
        {
            string[] slice = script_file.Split(Path.DirectorySeparatorChar);
            parent = Path.Combine("DATA3", slice[0], slice[1].Substring(0, slice[1].Length - 4));
        }
        else
        {
            parent = Path.Combine("DATA3", script_file.Split('.')[0]);
        }

        string scr = File.ReadAllText(Path.Combine("Script", script_file));
        if (!File.Exists(Path.Combine(parent, "Squares.txt")))
            return ret;
        string[] sq = File.ReadAllLines(Path.Combine(parent, "Squares.txt"));
    ...
    Match cir_match;
    Match rgb_match;
    private List<string> ScriptReader(string script_file) //возвращает список родителей
    {
        ...
        if (script_file.Split(Path.DirectorySeparatorChar).Length > 1)
        {
            string[] slice = script_file.Split(Path.DirectorySeparatorChar);
            parent = slice[0];
            code_page = CodePage.GetCodePage(int.Parse(parent), slice[1].Substring(0, slice[1].Length - 4), "import");
            original_code_page = CodePage.GetCodePage(int.Parse(parent), slice[1].Substring(0, slice[1].Length - 4));
            parent = Path.Combine("DATA3", parent, slice[1].Substring(0, slice[1].Length - 4));
        }
        else
        {
            parent = script_file.Split('.')[0];
            code_page = CodePage.GetCodePage(int.Parse(parent), "none", "import");
            original_code_page = CodePage.GetCodePage(int.Parse(parent));
            parent = Path.Combine("DATA3", parent);
        }

        StreamReader script_reader = new StreamReader(new FileStream("Script" + Path.DirectorySeparatorChar + script_file, FileMode.Open));
        ...
        while (cur_line != null)
        {
            ...
            string full_name = Path.Combine(parent, file_name);
            ...
            while (end_flag == false)
            {
                ...
                if (Regex.IsMatch(cur_line, border_pattern))
                {
                    ...
                    cur_line = script_reader.ReadLine();
                    if (Regex.IsMatch(cur_line, end_pattern))
                    {     // это если граница а за ней конец фалйла.
                        writer.Flush();
                        writer.Close();
                        end_flag = true;
                        cur_line = script_reader.ReadLine();
