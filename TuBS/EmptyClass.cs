...
public sealed partial class MainWindow
{
    ...
    {
        string out_dir = "DATA3" + Path.DirectorySeparatorChar;
        ...
        foreach (string font in fonts)
        {
            ...
            Console.WriteLine($"Converting ttx to png: {font} to {outpath}");
            ImageConv.TTXToPNG(font, outpath);
        }

        string[] pics = new string[] {
            Path.Combine ("DATA3", "13", "icon07.ar", "tex.ttx"),
            Path.Combine ("DATA3", "13", "misswp.ar", "kabe122.ttx"),
            Path.Combine ("DATA3", "13", "misswp.ar", "kabe123.ttx"),
            Path.Combine ("DATA3", "13", "misswp.ar", "kabe124.ttx"),
            Path.Combine ("DATA3", "13", "misswp2.ar", "kabe130.ttx"),
            ...
            Path.Combine ("DATA3", "2931", "kabe126.ttx"),
            Path.Combine ("DATA3", "2932", "kabe131.ttx"),
            Path.Combine ("DATA3", "2939", "kabe130.ttx"),
            Path.Combine ("DATA3", "2940", "kabe122.ttx"),
            Path.Combine ("DATA3", "2940", "kabe123.ttx"),
            Path.Combine ("DATA3", "2940", "kabe124.ttx"),
            Path.Combine ("DATA3", "2962", "icon02.ar", "tex.ttx"),
            Path.Combine ("DATA3", "2962", "icon03.ar", "tex.ttx"),
            Path.Combine ("DATA3", "2962", "icon04.ar", "tex.ttx"),
            Path.Combine ("DATA3", "2962", "icon05.ar", "tex.ttx"),
            Path.Combine ("DATA3", "2962", "icon07.ar", "tex.ttx"),
            Path.Combine ("DATA3", "2962", "iconwp10.ar", "tex.ttx"),
            Path.Combine ("DATA3", "2962", "iconwp11.ar", "tex.ttx"),
            Path.Combine ("DATA3", "2962", "iconwp13.ar", "tex.ttx"),
            Path.Combine ("DATA3", "2962", "iconwp15.ar", "tex.ttx"),
            Path.Combine ("DATA3", "2962", "iconwp17.ar", "tex.ttx"),
            Path.Combine ("DATA3", "2916")
        };
        foreach (string pic in pics)
        {
            ...
            string outpath = pic_dir + slice[slice.Length - 1] + ".png";
            Directory.CreateDirectory(Path.GetDirectoryName(outpath));
            Console.WriteLine($"Converting ttx to png: {pic} to {outpath}");
        ...
        {
            var tb = Path.Combine(out_dir, screen, "000.tb");
            var tp = Path.Combine(out_dir, screen, "000.tp");
            var outpath = Path.Combine(pic_dir, screen + ".png");
    ...
    private void PrintImportFiles()
    {
        ...
        foreach (string item in script_list)
            Console.WriteLine("Script: " + item);
        foreach (string item in sdat_list)
            Console.WriteLine("Script (dat): " + item);
        foreach (string item in ttx_list)
            Console.WriteLine("Picture (TTX): " + item);
        foreach (string item in tb_list)
            Console.WriteLine("Picture (Tb): " + item);
    ...
    protected void Pack(string child, string father)
    {
        ...
        for (int index = 0; index < num_of_files; ++index)
        {
            if (child_num == index)
            {
                ...
                child_reader.Close();
                byte[] buffer;
                if (protect[index] == 1 && compress_flag == 0)
